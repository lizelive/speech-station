...
// set current directory to the assembly directory
...
async Task ShowTranscriptAsync()
{
    ...
    {
        if (transcription is not RealtimeSegmentRecognized)
        {
            continue;
        }
...
IRealtimeSpeechTranscriptorFactory GetEchoSharpTranscriptorFactory(ISpeechTranscriptorFactory speechTranscriptorFactory, IVadDetectorFactory vadDetectorFactory)
{
    ...
    {
        // PaddingDuration = TimeSpan.FromSeconds(0.125), // The padding duration to add to the start and end of the segment. The default is 0.5 seconds.
    ...
    }, vadDetectorOptions: new VadDetectorOptions(){
        MinSilenceDuration = TimeSpan.FromSeconds(0.3),
    });
}
...
IVadDetectorFactory GetSileroVadDetector()
{
    ...
    return new SileroVadDetectorFactory(new SileroVadOptions(sileroOnnxPath)
    {
        ...
        // ThresholdGap = 0.4f, // The threshold gap for Silero VAD. The default is 0.15f.
    });
}
